...
namespace ns_menu
{
	...
	void checkbox(sol::this_state s, const std::string& name)
	{
	...
	void combobox(sol::this_state s, std::string name, std::vector <std::string> labels) //-V813
	{
		if (labels.empty())
			return;
	...
	void slider_int(sol::this_state s, const std::string& name, int min, int max)
	{
		auto script = get_current_script(s);
		auto script_id = c_lua::get().get_script_id(script);
	...
	void slider_float(sol::this_state s, const std::string& name, float min, float max)
	{
		auto script = get_current_script(s);
		auto script_id = c_lua::get().get_script_id(script);
	...
	void colorpicker(sol::this_state s, const std::string& name)
	{
		auto script = get_current_script(s);
		auto script_id = c_lua::get().get_script_id(script);
...
void c_lua::initialize()
{
	...
	menu[crypt_str("add_check_box")] = ns_menu::checkbox;
	menu[crypt_str("add_combo_box")] = ns_menu::combobox;
	menu[crypt_str("add_slider_int")] = ns_menu::slider_int;
	menu[crypt_str("add_slider_float")] = ns_menu::slider_float;
	menu[crypt_str("add_color_picker")] = ns_menu::colorpicker;
