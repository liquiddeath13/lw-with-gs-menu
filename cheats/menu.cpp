	...
	
	{
		if (!sub_tab_rage)
		{
			ui::BeginChild(crypt_str("Aimbot"), ImVec2(250, 260));
			{
				...
				{
					ui::Checkbox(crypt_str("Defensive"), &g_cfg.ragebot.defensive_doubletap);
		...
		ui::BeginChild(crypt_str("Settings"), ImVec2(250, 260));
		{
			...
			if (!players.empty())
			{
				...
				if (!g_cfg.player_list.white_list[players.at(current_player).i]) //-V106
				{
					...
					ui::Checkbox(crypt_str("Force body yaw"), &g_cfg.player_list.playerlist_forceyaw[players.at(current_player).i]); //-V106
					if (g_cfg.player_list.playerlist_forceyaw[players.at(current_player).i])
					{
						ui::slider
					}
					ui::Checkbox(crypt_str("Force pitch"), &g_cfg.player_list.playerlist_forcepitch[players.at(current_player).i]); //-V106
