...
void lagcompensation::update_player_animations(player_t* e)
{
	...
	if (!player_info.fakeplayer && g_ctx.local()->is_alive() && e->m_iTeamNum() != g_ctx.local()->m_iTeamNum() && !g_cfg.legitbot.enabled) //-V807
	{
		...
		if (g_cfg.player_list.playerlist_bodyyaw[e->EntIndex()])
		{
			//e->m_angEyeAngles().y animstate->m_flGoalFeetYaw
			animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + sizeof(g_cfg.player_list.playerlist_forceyaw));
		}
		...
		if (g_cfg.player_list.playerlist_bodyyaw[e->EntIndex()])
		{
			...
			{
			case RESOLVER_FIRST:
				record->side = RESOLVER_PLAYERLIST;
				break;
			case RESOLVER_SECOND:
				record->side = RESOLVER_PLAYERLIST;
				break;
			...
			//g_cfg.player_list.playerlist_forceyaw
			/*switch (record->side)
			{
			case RESOLVER_ORIGINAL:
				animstate->m_flGoalFeetYaw = math::normalize_yaw(e->m_angEyeAngles().y + sizeof(g_cfg.player_list.playerlist_forceyaw));
