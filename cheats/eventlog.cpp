...
void Game_Msg(const char* msg, ...)
{
	...
	m_cvar()->ConsoleColorPrintf(Color(255, 0, 0), "[gamesense] ");
...
void Features::CEventLogger::Draw_D3D9()
{
	...
	for (auto& l : logs)
	{
		render::get().text(fonts[LOGS], 5, 5 + offset + l.yOffset, Color(117, 187, 253), HFONT_CENTERED_NONE, "[gamesense]  ");
		render::get().text(fonts[LOGS], 5 + 40 + 5, 5 + offset + l.yOffset, Color(255, 255, 255), HFONT_CENTERED_NONE, l.text.c_str());
		offset += textSize + l.yOffset;
		
	}
	if (logs.size())
		alert::push(logs.at(logs.size()-1).text);
}