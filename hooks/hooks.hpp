...
namespace hooks
{
	...
	extern vmthook* effects;
	extern vmthook* sv_cheats;
	typedef int(__thiscall* svc_get_bool_t) (PVOID);
	...
	extern void __stdcall new_hooked_createmove(int sequence_number, float input_sample_frametime, bool active, bool& bSendPacket);
	__declspec (noinline) void __stdcall TEFireBulletsPostDataUpdate_h(DataUpdateType_t updateType);
	void __stdcall FireBullets_PostDataUpdate(C_TEFireBullets* thisptr, DataUpdateType_t updateType);
	int __fastcall hk_sv_teamid_overhead_get_bool(const PVOID con_var, void* edx);
...
namespace alert
{
	...
	extern void setup_element();
	extern void check();
	extern void push(const std::string_view text);
