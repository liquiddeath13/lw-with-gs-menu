...
public:
	...
	{
		static auto oFindElement = reinterpret_cast<CHudElement * (__thiscall*)(CHud*, const char*)>
			(util::FindSignature("client.dll", "55 8B EC 53 8B 5D 08 56 57 8B F9 33 F6 39"));
...
inline uintptr_t follow_relative_jump(const uintptr_t address)
{
...
struct CPanel2D
{
	...
	{
		static auto fn = reinterpret_cast<void(__thiscall*)(CPanel2D*, const char*, int)>(util::FindSignature("client.dll", "55 8B EC A1 ? ? ? ? 83 EC 08 56 8B F1 57 A8 01 75 26 8B 0D ? ? ? ? 83 C8 01 A3 ? ? ? ? 68 ? ? ? ? 8B 01 FF 90 ? ? ? ? 66 A3 ? ? ? ? A1"));
		return fn(this, text, mode);
	}
	void HidePanel()
	{
		static auto fn = reinterpret_cast<void(__thiscall*)(CPanel2D*, bool)>(follow_relative_jump((util::FindSignature("client.dll", "E8 ? ? ? ? 5F 5E 5B 8B E5 5D C2 04 00 8B D3 B9 ? ? ? ? E8 ? ? ? ? 85 C0 0F 85 ? ? ? ? 8B 44 24 14"))));
		return fn(this, false);
	}
};


class CCSGO_HudReticle : public CHudElement
{
public:
	char pad_0x0000[0x84]; //0x0000
	int m_iPlayersRendered; //0x0084 
	CPanel2D* GetPanel2D()
	{
...
void __fastcall hooks::hooked_painttraverse(void* ecx, void* edx, vgui::VPANEL panel, bool force_repaint, bool allow_force)
{
	...
	static auto hud = *reinterpret_cast<CHud**>(util::FindSignature("client.dll", "B9 ? ? ? ? 68 ? ? ? ? E8 ? ? ? ? 89 46 24") + 0x1);
	static auto elem = reinterpret_cast<CCSGO_HudReticle*>(hud->FindElement("CCSGO_HudReticle"));
	elem->m_iPlayersRendered = 0;
	...
	if (reload_fonts())
	{
		...
		fonts[FONT_SHONAX] = create_font(crypt_str("Source Code Pro Medium"), 15, FW_MEDIUM, FONTFLAG_OUTLINE);
...
namespace alert
{
	CCSGO_HudReticle* m_element = nullptr;
...
#undef max
	...
	void setup_element()
	{
		static auto hud = *reinterpret_cast<CHud**>(util::FindSignature("client.dll", "B9 ? ? ? ? 68 ? ? ? ? E8 ? ? ? ? 89 46 24") + 0x1);
		if (!m_element)
			m_element = reinterpret_cast<CCSGO_HudReticle*>(hud->FindElement("CCSGO_HudUniqueAlerts"));
	}

	void check()
	{
		if (m_globals()->m_curtime > m_last_push + m_timeout)
		{
	...
	void push(const std::string_view text)
	{
		...
		m_last_push = m_globals()->m_curtime;
	}
}